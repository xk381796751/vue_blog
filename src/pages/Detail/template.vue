	...
	<div id="detail">
		...
		<section class="article">
			...
			<p>有两个东西，启发我写这篇指南。 第一个是雷切尔·安德鲁（Rachel Andrew）的书<a href="http://abookapart.com/products/get-ready-for-css-grid-layout"
				 ...
				 target="_blank">Flexbox完全指南</a>，当需要查阅 flexbox 的一切资料时我就会找这篇文章。 这篇文章帮助了很多人学习 Flex 布局，也是 Google
				上搜索“flexbox”关键字排名第一的文章。你会发现他的文章和我的很多相似之处，有最好的范例在那放着为什么咱不偷师学着写呢？</p>
			<p>本指南的目的是介绍网格概念，因为它们存在于最新版本的规范中。 因此我不会覆盖过时的IE语法，而且随着规范的成熟，我会尽最大努力保存更新本指南。</p>
			<h2 id="基础知识以及浏览器支持情况">基础知识以及浏览器支持情况</h2>
			<p>一开始你需要使用<code>display：grid</code>把容器元素定义为一个网格，使用<code>grid-template-columns</code>和<code>grid-template-rows</code>设置列和行大小，然后使用<code>grid-column</code>
			...
			<p>截至2017年3月，许多浏览器都提供了原生的、不加前缀的对CSS Grid的支持，比如 Chrome（包括Android），Firefox，Safari（包括iOS）和Opera。 另一方面，Internet Explorer
				10和11支持它，但需要使用过时的语法。 Edge浏览器已经宣布将支持标准的Grid语法，但暂未支持。</p>
			<blockquote>
	  ...
	  <span class="hljs-tag">&lt;/<span class="hljs-name">div</span>&gt;</span>
			...
			<ul>
				...
				<li><strong>subgrid</strong> - 如果你的 grid container 本身就是一个 grid
					item（即,嵌套网格），你可以使用这个属性来表示你想从它的父节点获取它的行/列的大小，而不是指定它自己的大小。</li>
			</ul>
		...
		<span class="hljs-attribute">display</span>: grid | inline-grid | subgrid;
			...
			<p>注意：<code>column</code>, <code>float</code>, <code>clear</code>, 以及 <code>vertical-align</code> 对一个 grid container
				没有影响</p>
		  ...
		  <span class="hljs-string">"footer footer footer footer"</span>;
			...
			<ul>
				...
				<li><code>grid-template-rows</code> / <code>&lt;grid-template-columns</code> - 把 grid-template-columns 和
					grid-template-rows 设置为指定值, 与此同时, 设置 grid-template-areas 为 none</li>
			</ul>
			<pre><code class="lang-css"><span class="hljs-selector-class">.container</span> {
	  ...
	  <span class="hljs-selector-class">.item-e</span> {
		...
		<span class="hljs-attribute">grid-auto-flow</span>: column;
			...
			<ul>
				...
				<li><code>&lt;grid-template-rows&gt;</code> / <code>&lt;grid-template-columns&gt;</code> - 将 grid-template-rows 和
					grid-template-columns 分别设置为指定值，将所有其他子属性设置为其初始值</li>
